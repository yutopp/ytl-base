#ifndef YTL_BASE_DETAIL_BUFFER_SHORT_BUFFER_IMPL_HPP
#define YTL_BASE_DETAIL_BUFFER_SHORT_BUFFER_IMPL_HPP
...
namespace ytl
{
    ...
    namespace detail
    {
        namespace
        {
            struct short_buffer_construct_tag {};
            YTL_CONSTEXPR auto const short_buffer_construct = short_buffer_construct_tag();

            struct short_buffer_construct_n_tag {};
            YTL_CONSTEXPR auto const short_buffer_construct_n = short_buffer_construct_n_tag();
        }
        ...
        inline YTL_CONSTEXPR auto to_binary_little( T const& val, ytl::cu::index_tuple<std::size_t, Indexes...> const& )
            -> short_buffer<typename boost::uint_value_t<sizeof(T)>::least, sizeof(T)>;
        ...
        public:
            ...
            template<typename... T>
            explicit YTL_CONSTEXPR short_buffer( short_buffer_construct_tag, T const... xs )
                : bin_{{ static_cast<value_type const>( xs )... }}
                , size_{ sizeof...( xs ) }
            {}

            template<typename... T>
            explicit YTL_CONSTEXPR short_buffer( short_buffer_construct_n_tag, size_type const size, T const... xs )
                : bin_{{ static_cast<value_type const>( xs )... }}
                , size_{ size }
            {}
...
//                size += sizeof(T);
        ...
        inline YTL_CONSTEXPR auto to_binary_little( T const& val, ytl::cu::index_tuple<std::size_t, Indexes...> const& )
            -> short_buffer<typename boost::uint_value_t<sizeof(T)>::least, sizeof(T)>
        {
            return short_buffer<typename boost::uint_value_t<sizeof(T)>::least, sizeof(T)>(
                        short_buffer_construct, 
        ...
        {
            return short_buffer<SizeType, MaxSize>( short_buffer_construct_n, t.size() + 1, element<Indexes>( t, val )... );
        }
                 ...
                 >
        {
            return concat_impl(
                        short_buffer<
                            ...
                            >::type,
                            ( MaxSize1 + MaxSize2 )
                        >( short_buffer_construct_n, p1.size(), p1[Indexes1]... ),
                        p2, ytl::cu::make_index_tuple<std::size_t, 0, MaxSize2>()
                        );
        }
        ...
        {
            return ToBuffer( short_buffer_construct_n, source.size(), source[Indexes]... );
        }
        ...
        private:
            typedef raw_little_endian_method                        write_method_type;
        ...
        public:
            ...
            short_buffer( short_buffer_construct_tag )
            {
                buffer_type arr = {};
...
# define DEF_CTOR(z, n, unused) \
            ...
            short_buffer( short_buffer_construct_tag, BOOST_PP_ENUM_BINARY_PARAMS(n, T, const p) ) \
            { \
...
# undef DEF_CTOR
        ...
        public:
                   ...
                   >
            {
                short_buffer<
                    ...
                    >::type,
                    ( MaxSize + MaxSizeRhs )
                > target( short_buffer_construct_n, lhs );
        ...
        {
            return ToBuffer( short_buffer_construct_n, source );
        }
...
#endif /*YTL_BASE_DETAIL_BUFFER_SHORT_BUFFER_IMPL_HPP*/